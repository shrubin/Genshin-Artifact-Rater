...
async def rate(ctx):
	...
	if options and validators.url(options[0]):
		...
		if not DEVELOPMENT:
			await ctx.send('Error: No image or url found, please make sure they were sent in the same message')
		return
	...
	try:
		...
		print(f'Error: Could not parse "{ctx.message.content}"')
		if not DEVELOPMENT:
			await ctx.send('Error: Command cannot be parsed, please double check the format and spelling')
		return
