...
class kor(translation):
	def __init__(self):
		...
		self.err = '에러'
		self.err_not_found = '에러: 이미지 혹은 url이 발견되지 않았습니다. 메시지 안에 포함되어있는지 확인해주세요.'
		...
		self.help_commands = {
			'rate': [
				...
				**매개 변수**
				`이미지/url`
				등급을 매길 이미지입니다. 파일로 첨부하거나 메시지에 URL을 첨부하면 됩니다. [예시]({self.SAMPLE_URL})

				`프리셋`
				사용할 가중치의 사전 설정. `-presets` 명령어로 사용 가능한 프리셋을 볼 수 있거나, `-help`로 어떻게 설정하는지 볼 수 있습니다.

				`레벨`
				비교할 성유물의 레벨(0~20)입니다. 만약 레벨 자동 탐지가 부정확하면, 이것으로 레벨을 고칠 수 있습니다.
		...
		**Config**
		...
		`{self.help_commands['lang'][0]}`
		모든 명령에 대한 언어를 2-문자 언어 코드 `lang` 으로 설정합니다.
		플래그 반응을 눌러서 언어를 변경할 수도 있습니다.

		`{self.help_commands['preset'][0]}`
		성유물 등급을 매길 때 사용할 프리셋을 만듭니다. 프리셋이 사용될 때 `가중치`는 `-rate` 명령어에 사용됩니다.
